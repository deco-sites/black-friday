...
export default function Newsletter(props: Props) {
  const { title, description, form, layout,image } = { ...DEFAULT_PROPS, ...props };
  ...
  const headerLayout = (
    <div class={` flex flex-col lg:${layout?.content?.alignment === "Center" ? alignment.center : alignment.left } ${alignment.center} gap-2 `} >
        <h3 class={"font-bold text-sm lg:text-2xl text-[#191919] "}>{title}</h3>
        <p class={"bg:text-base text-xs text-center lg:text-start text-[#191919]"}>{description}</p>
    </div>
    
  );

  const formLayout = form && (
    <form  onSubmit={handleSubmit} class="flex flex-col  items-center w-full lg:max-w-[422px] gap-4">
  ...
  return (
      ...
      class={` container lg:max-w-[1200px] p-4  flex flex-col lg:flex-row w-full  justify-center gap-20 items-center lg:mt-20 lg:pr-4 ${
        bordered
          ? isReverse ? "bg-[#F5F5F5]" : "bg-secondary"
          : bgLayout
      } ${bordered ? "p-4 " : "p-0"}`}
    >
